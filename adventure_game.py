...
# Location Class Definition: Represents a place in the game world
class Location:
    def __init__(self, name, description, enemies, items):
        ...
        self.enemies = enemies  # List of enemies that might be encountered in this location
        self.items = items  # List of items that might be found in this location

    def explore(self):
        # The method for exploring the location
        ...
        if action == "1":
            # If the player chooses to explore, either encounter enemies or find items
            if self.enemies:
                # If enemies are present, one is randomly selected for battle
                enemy = random.choice(self.enemies)
                print(f"\nA wild {enemy.name} appears!")
                return enemy  # Return the enemy to initiate combat
            else:
                # If no enemies, player finds items instead
                print("No enemies are around. You find some items instead.")
                return self.items  # Return the list of items found
        else:
            # If the player leaves the location
            print("You leave the location.")
            return None  # Return None to indicate the player is leaving
...
def combat(player, enemy, location_items):
    ...
    while player.health > 0 and enemy.health > 0:
        # Combat loop where both the player and enemy take turns
        print(f"\nYour health: {player.health}")
        print(f"{enemy.name}'s health: {enemy.health}")
        
        # Player's turn: Choose an action (attack, defend, use item, flee)
        print("\nChoose an action:")
        print("[1] Attack")
        print("[2] Defend")
        print("[3] Use Item")
        ...
        if action == "1":  # Attack
            ...
            # Add chance for critical hit (10% chance)
            ...
            if player.inventory:
                ...
                item.effect(player)  # Apply the item's effect
                player.inventory.remove(item)  # Remove used item from inventory
            else:
                print("\nYou have no items left!")
        elif action == "4":  # Flee
            # Attempt to flee (50% chance of success)
        ...
        # Enemy's turn: The enemy attacks if it's still alive
        if enemy.health > 0:
            if random.random() < 0.5:  # 50% chance for enemy to attack
                damage = random.randint(5, 15) + enemy.attack - player.defense
                damage = max(damage, 0)  # Prevent negative damage
                if player.defending:
                    # Reduce damage if the player is defending
    ...
    if player.health <= 0:
        ...
        player.add_item(item)  # Add item to player's inventory
...
class Item:
    def __init__(self, name, type_, effect):
        ...
        self.type = type_  # Type of the item (healing, weapon, etc.)
        self.effect = effect  # Function that applies the item's effect to the player

class Enemy:
    def __init__(self, name, health, attack, defense, speed):
        self.name = name  # Name of the enemy
        self.health = health  # Enemy's health
        self.attack = attack  # Enemy's attack power
        self.defense = defense  # Enemy's defense
        self.speed = speed  # Enemy's speed (not currently used in combat)

class Character:
    def __init__(self, name, char_class):
        self.name = name  # Player's name
        self.char_class = char_class  # Player's class (e.g., Paladin, Necromancer)
        self.health = 100  # Default health
        self.attack = 10  # Default attack
        self.defense = 5  # Default defense
        self.speed = 10  # Default speed
        self.inventory = []  # Player's inventory (list of items)
        self.defending = False  # Whether the player is currently defending in combat
    ...
    def set_class_stats(self):
        ...
        # Predefined stats for different character classes
        class_stats = {
        ...
        stats = class_stats.get(self.char_class, {})  # Get the stats for the chosen class
